...
local function resolveLogPath()
    ...
    source = source:gsub("^@", "")
    local modDirectory = source:match("^(.*)[/\\]Scripts[/\\]session_log%.lua$")
    if modDirectory == nil then return nil end
    return modDirectory .. "/AccessorySlotsResearch-session.log"
end
...
function sessionLog.append(line)
    ...
    local ok = pcall(function()
        logHandle:write(os.date("%Y-%m-%d %H:%M:%S"), " | ", tostring(line), "\n")
        logHandle:flush()
    end)
    return ok
end
